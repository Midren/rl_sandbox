...
from rl_sandbox.utils.rollout_generation import (collect_rollout, collect_rollout_num, iter_rollout_async, iter_rollout,
...
def main(cfg: DictConfig):
    ...
    buff = ReplayBuffer()
    fillup_replay_buffer(env, buff, max(cfg.training.prefill, cfg.training.batch_size), policy_agent.preprocess_obs)
    ...
    while global_step < cfg.training.steps:
        ...
        for s, a, r, n, f, _ in iter_rollout(env, agent, policy_agent.preprocess_obs):
        ...
        if global_step % cfg.training.val_logs_every == 0:
            ...
            if cfg.validation.visualize:
                rollouts = collect_rollout_num(env, 1, policy_agent.preprocess_obs, agent, collect_obs=True)
